...
export class SmsService {
  ...
  private initializeProviders(): void {
    ...
    const vonageApiKey = this.configService.get<string>('VONAGE_API_KEY') || this.configService.get<string>('NEXMO_API_KEY');
    const vonageApiSecret = this.configService.get<string>('VONAGE_API_SECRET') || this.configService.get<string>('NEXMO_API_SECRET');
  ...
  async sendSMS(message: SMSMessage): Promise<SMSDeliveryResult> {
    ...
    for (const provider of this.providerPriority) {
      try {
        ...
        if (result.success) {
          this.logger.log(`✅ SMS sent successfully via ${provider} to ${message.to} (${responseTime}ms)`);
          return result;
        }
      } catch (error) {
        const responseTime = Date.now() - startTime;
   ...
   */
  private async sendWithProvider(provider: string, message: SMSMessage): Promise<SMSDeliveryResult> {
    switch (provider) {
      case 'twilio':
  ...
  async testSMS(phoneNumber: string, customMessage?: string): Promise<SMSDeliveryResult> {
    const message: SMSMessage = {
      to: phoneNumber,
      body: customMessage || `Test SMS from MASH Device Monitoring System at ${new Date().toISOString()}`,
    };
