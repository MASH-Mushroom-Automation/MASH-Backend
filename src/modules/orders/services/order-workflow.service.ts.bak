...
export class OrderWorkflowService {
  ...
  async createFromCart(userId: string, shippingAddress?: any): Promise<any> {
    return this.tracer.startActiveSpan('createFromCart', async span => {
      const timer = this.prometheus.startTimer('order_creation_duration_seconds');
      ...
      try {
        ...
        // 7. Record metrics
        this.prometheus.incrementCounter('orders_created_total', { source: 'cart' });
        timer();
        ...
        this.prometheus.incrementCounter('order_creation_errors_total', {
  ...
  async createDirect(createOrderDto: CreateOrderDto, currentUser: any): Promise<any> {
    return this.tracer.startActiveSpan('createDirect', async span => {
      ...
      try {
        ...
        // 6. Record metrics
        this.prometheus.incrementCounter('orders_created_total', { source: 'direct' });
        timer();
  ...
  /**
   * Calculate order pricing before creation
   */
   ...
   */
  private calculatePricing(cart: any): any {
